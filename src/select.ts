...
export type Select<SQL extends string, Source extends Object> =
...
export type InferSelect<SQL extends string> =
	TrimLeft<SQL> extends `${
		| 'SELECT'
		| 'select'}${_}${infer Fields}${_}${'FROM' | 'from'}${_}${infer TableOrSubQuery}`
		? ExtractTableAlias<TableOrSubQuery> extends infer Table extends
				ExtractTableAliasResult
			? Table &
					ExtractSelectFields<Fields> &
					(Table['next'] extends string
						? InferJoin<Table['next']>
						: {})
			: never
		: never
...
type InferJoin<JoinClause extends string> =
	JoinClause extends `${JoinOperator}${_}JOIN${_}${infer JoinConstraint}`
		? {
				join: ExtractJoinConstraintTableAlias<JoinConstraint>
			}
		: JoinClause extends `JOIN${_}${infer JoinConstraint}`
			? {
					join: ExtractJoinConstraintTableAlias<JoinConstraint>
				}
			: JoinClause extends `${string}${_}JOIN${_}${infer JoinConstraint}`
				? {
						join: ExtractJoinConstraintTableAlias<JoinConstraint>
					}
				: {}
